...
- **No persistent state file.** Nothing is remembered but the last toast's id, in the
  shell's memory. A running TUI listens for thread and Screener requests on a mode-0600,
  instance-named socket in `$XDG_RUNTIME_DIR/hey-cli`, with an owner-only temporary
  directory as the fallback, and removes it when the TUI exits; turning toasts on is nothing more than
