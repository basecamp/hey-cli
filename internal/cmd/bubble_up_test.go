...
const (
	bubbleTestPostingID int64 = 731245
	bubbleTestTopicID   int64 = 912876
)
...
func (s *bubbleTestService) writeImboxPage(w http.ResponseWriter, page string) {
	s.t.Helper()
	switch page {
	case "target":
		s.writeBoxPayload(w, "imbox", true, false, bubbleTestTopicID, "")
	case "incomplete":
		s.writeBoxPayload(w, "imbox", false, false, bubbleTestTopicID, s.server.URL+"/imbox.json?page=still-incomplete")
	default:
		if page != "" {
			s.t.Errorf("unexpected Imbox page %q", page)
			w.WriteHeader(http.StatusNotFound)
			return
		}
		s.writeBox(w, "imbox")
...
func (s *bubbleTestService) writeBox(w http.ResponseWriter, kind string) {
	...
	case "imbox":
		...
		if s.state == bubbleStatePaginated {
			nextURL = s.server.URL + "/imbox.json?page=target"
		}
		if s.state == bubbleStateIncomplete {
			nextURL = s.server.URL + "/imbox.json?page=incomplete"
		}
	case "bubblebox":
...
func TestBubbleActionsHandleScheduledBoxItem(t *testing.T) {
	for _, command := range []string{"bubble-up-now", "pop"} {
		t.Run(command, func(t *testing.T) {
			...
			if err != nil {
				t.Fatalf("%s scheduled box item: %v", command, err)
			}
			result := decodeBubbleResult(t, response)
...
func TestBubbleUpNowDoesNotReplayUnconfirmedMutation(t *testing.T) {
	...
	if got := apierr.AsError(err).Code; got != "mutation_unconfirmed" {
...
func TestBubbleActionsFailClosedOnTargetMismatchOrAbsence(t *testing.T) {
	for _, tc := range []struct {
		...
		{name: "incomplete box item", state: bubbleStateIncompletePosting, wantCode: "target_incomplete"},
		{name: "missing", state: bubbleStateMissing, wantCode: apierr.CodeNotFound},
	} {
		t.Run(tc.name, func(t *testing.T) {
			service := newBubbleTestService(t, tc.state)
			defer service.close()
			...
			if got := apierr.AsError(err).Code; got != tc.wantCode {
...
func TestBubbleTargetLookupPrefersExactPairWhenAnotherBoxItemSharesThread(t *testing.T) {
	service := newBubbleTestService(t, bubbleStateSharedTopic)
	defer service.close()

	response, err := runBubbleCommand(t, service.server.URL, "bubble-up-now")
	if err != nil {
		t.Fatalf("bubble-up-now with shared thread: %v", err)
	}
	result := decodeBubbleResult(t, response)
	...
	nowPOST, popDELETE := service.counts()
	if nowPOST != 1 || popDELETE != 0 {
		t.Errorf("shared-thread requests now/pop = %d/%d", nowPOST, popDELETE)
	}
}
...
func TestBubbleTargetLookupRejectsIncompletePagination(t *testing.T) {
	...
	if got := apierr.AsError(err).Code; got != "target_lookup_incomplete" {
...
func TestBubbleActionsRequirePositiveExactIDs(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "missing topic", args: []string{"bubble-up-now", strconvInt64(bubbleTestPostingID)}},
		{name: "zero box item", args: []string{"bubble-up-now", "0", "--topic-id", strconvInt64(bubbleTestTopicID)}},
		{name: "nonnumeric box item", args: []string{"bubble-up-now", "nope", "--topic-id", strconvInt64(bubbleTestTopicID)}},
		{name: "zero topic", args: []string{"pop", strconvInt64(bubbleTestPostingID), "--topic-id", "0"}},
	} {
		t.Run(tc.name, func(t *testing.T) {
			...
			if got := apierr.AsError(err).Code; got != apierr.CodeUsage {
