...
func TestPollIgnoresRepositoryLocalConfig(t *testing.T) {
	repo := t.TempDir()
	if err := os.MkdirAll(filepath.Join(repo, ".hey"), 0o755); err != nil {
		t.Fatal(err)
	}
	...
	// The plugin runs the poll from the shell's working directory, wherever
	// that is: an untrusted checkout must neither fail it at the trust gate
	// nor point it at the checkout's server.
	resp, err := runPoll(t, t.TempDir(), server.URL, true)
	if err != nil || !resp.OK {
		t.Errorf("the poll must answer from an untrusted checkout, got %+v, %v", resp, err)
	}
	if cfg.UntrustedLocalConfig() != nil || cfg.SourceOf("base_url") != config.SourceFlag {
		t.Errorf("the poll must load global configuration only, got base_url from %v", cfg.SourceOf("base_url"))
	}
